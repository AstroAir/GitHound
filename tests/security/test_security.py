...
class TestAuthentication:
    ...
    def test_token_without_required_claims(self, api_client, auth_manager) -> None:
        ...
        incomplete_tokens = [
            {"exp": datetime.utcnow() + timedelta(hours=1)},  # Missing sub, username, roles
            {"sub": "user", "exp": datetime.utcnow() + timedelta(hours=1)},  # Missing username, roles
            {"sub": "user", "username": "user", "exp": datetime.utcnow() + timedelta(hours=1)},  # Missing roles
        ]
