...
from githound.schemas import (
    OutputFormat, FilterOperator, DataFilter, SortCriteria, SortOrder,
    ExportOptions, AuthorSchema, CommitSchema, SearchResultSchema
)
from githound.models import (
    SearchResult, CommitInfo, SearchType, BranchInfo, TagInfo, 
...
class TestSchemas:
    ...
    def test_commit_schema_creation(self, sample_commit_info):
        ...
        author = AuthorSchema(
            name=sample_commit_info.author_name,
            email=sample_commit_info.author_email
        )
        committer = AuthorSchema(
            name=sample_commit_info.committer_name,
            email=sample_commit_info.committer_email
        )
...
class TestExportManager:
    ...
    def test_filter_evaluation(self, sample_search_results):
        ...
        assert filtered_results[0].matching_line is not None and "function" in filtered_results[0].matching_line
