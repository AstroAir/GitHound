...
class TestWebServer:
    ...
    def test_serve_command_with_workers_no_reload(self, mock_uvicorn_run) -> None:
        ...
        result = self.runner.invoke(app, ["serve", "--workers", "4"])
    ...
    def test_dev_command(self, mock_serve) -> None:
        ...
        mock_serve.assert_called_once_with(
            host="127.0.0.1", port=8000, reload=True, log_level="debug", workers=1
        )
    ...
    def test_prod_command_defaults(self, mock_serve) -> None:
        ...
        assert result.exit_code == 0
        mock_serve.assert_called_once_with(
            host="0.0.0.0", port=8000, reload=False, log_level="info", workers=4
        )
    ...
    def test_prod_command_custom_params(self, mock_serve) -> None:
        ...
        result = self.runner.invoke(
            app, ["prod", "--host", "192.168.1.100", "--port", "9000", "--workers", "8"]
        )

        assert result.exit_code == 0
        mock_serve.assert_called_once_with(
            host="192.168.1.100", port=9000, reload=False, log_level="info", workers=8
        )
