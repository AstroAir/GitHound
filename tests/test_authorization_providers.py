...
class TestEunomiaAuthorizationProvider:
    ...
    def test_eunomia_provider_creation(self, mock_base_provider, policy_file) -> None:
        ...
        try:
            ...
            provider = EunomiaAuthorizationProvider(
                base_provider=mock_base_provider, policy_file=policy_file, server_name="test-server"
            )
    ...
    @pytest.mark.asyncio
    async def test_eunomia_authentication_passthrough(
        self, mock_base_provider, policy_file
    ) -> None:
        ...
        try:
            ...
            provider = EunomiaAuthorizationProvider(
                base_provider=mock_base_provider, policy_file=policy_file
            )
    ...
    async def test_eunomia_permission_checking(self, mock_base_provider, policy_file) -> None:
        ...
        try:
            ...
            assert (
                await provider.check_permission(readonly_user, "read", "test-server:info") is True
            )
            assert (
                await provider.check_permission(readonly_user, "write", "test-server:repo") is False
            )
    ...
    async def test_permit_authentication_passthrough(self, mock_base_provider) -> None:
        ...
        try:
            ...
            with patch("githound.mcp.auth.providers.permit.PermitMcpMiddleware"):
                provider = PermitAuthorizationProvider(
                    base_provider=mock_base_provider, permit_api_key="test-api-key"
                )
    ...
    async def test_permit_permission_checking(self, mock_base_provider) -> None:
        ...
        try:
            ...
            with patch("githound.mcp.auth.providers.permit.PermitMcpMiddleware"):
                ...
                # Readonly should have limited access
                assert (
                    await provider.check_permission(readonly_user, "read", "test-server:info")
                    is True
                )
                assert (
                    await provider.check_permission(readonly_user, "write", "test-server") is False
                )
    ...
    async def test_permit_tool_permission_checking(self, mock_base_provider) -> None:
        ...
        try:
            ...
            with patch("githound.mcp.auth.providers.permit.PermitMcpMiddleware"):
                ...
                assert (
                    await provider.check_tool_permission(
