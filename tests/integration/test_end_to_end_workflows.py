...
class TestCompleteWorkflows:
    ...
    async def test_complete_repository_analysis_workflow(self, integration_test_repo):
        ...
        from githound.mcp_server import analyze_repository, RepositoryInput
        
        context = MockMCPContext()
        repo_input = RepositoryInput(repo_path=temp_dir)
        
        analysis_result = await analyze_repository(repo_input, context)
        ...
        assert metadata['total_branches'] >= 2  # main + feature branch
        assert metadata['total_tags'] >= 1  # v1.0.0 tag
        
        # Step 2: Commit History Analysis
        from githound.mcp_server import get_commit_history, CommitHistoryInput
        
        history_input = CommitHistoryInput(
            repo_path=temp_dir,
            max_count=10
        )
        
        history_result = await get_commit_history(history_input, context)
        ...
        from githound.mcp_server import get_file_blame, FileBlameInput
        ...
        blame_result = await get_file_blame(blame_input, context)
        ...
        from githound.mcp_server import get_author_stats, AuthorStatsInput
        
        stats_input = AuthorStatsInput(repo_path=temp_dir)
        stats_result = await get_author_stats(stats_input, context)
        ...
        # Step 5: Data Export
        from githound.mcp_server import export_repository_data, ExportInput
        ...
        export_result = await export_repository_data(export_input, context)
        ...
        with open(export_file, 'r') as f:
            ...
            assert exported_data['total_commits'] >= 4
        
        # Verify context messages were logged
        assert len(context.messages) > 0
        assert any("Analyzing repository" in msg for msg in context.messages)
    ...
    async def test_error_handling_workflow(self, integration_test_repo):
        ...
        result = await analyze_repository(invalid_input, context)
        ...
        blame_result = await get_file_blame(invalid_blame_input, context)
        assert blame_result['status'] == 'error'
        
        # Test 3: Invalid commit hash for comparison
        from githound.mcp_server import compare_commits, CommitComparisonInput
        
        invalid_comparison_input = CommitComparisonInput(
            repo_path=temp_dir,
            from_commit="invalid_hash_1",
            to_commit="invalid_hash_2"
        )
        
        comparison_result = await compare_commits(invalid_comparison_input, context)
    ...
    async def test_concurrent_operations_workflow(self, integration_test_repo):
        ...
        from githound.mcp_server import analyze_repository, RepositoryInput
        context1 = MockMCPContext()
        repo_input = RepositoryInput(repo_path=temp_dir)
        tasks.append(analyze_repository(repo_input, context1))
        
        # Task 2: Commit history
        from githound.mcp_server import get_commit_history, CommitHistoryInput
        context2 = MockMCPContext()
        history_input = CommitHistoryInput(repo_path=temp_dir, max_count=5)
        tasks.append(get_commit_history(history_input, context2))
        
        # Task 3: Author statistics
        from githound.mcp_server import get_author_stats, AuthorStatsInput
        context3 = MockMCPContext()
        stats_input = AuthorStatsInput(repo_path=temp_dir)
        tasks.append(get_author_stats(stats_input, context3))
        ...
        for i, result in enumerate(results):
            ...
            assert result['status'] == 'success', f"Task {i} failed: {result.get('error', 'Unknown error')}"
    ...
    async def test_export_workflow_multiple_formats(self, integration_test_repo):
        ...
        for format_type in formats:
            ...
            result = await export_repository_data(export_input, context)
...
class TestResourceIntegration:
    ...
    async def test_resource_access_workflow(self, integration_test_repo):
        ...
        from githound.mcp_server import (
            get_repository_config, get_repository_contributors, get_repository_summary
        )
        
        context = MockMCPContext()
        
        # Test repository config resource
        config_result = await get_repository_config(temp_dir, context)
        ...
        contributors_result = await get_repository_contributors(temp_dir, context)
        assert isinstance(contributors_result, str)
        assert "Repository Contributors" in contributors_result
        assert "commits" in contributors_result.lower()
        
        # Test summary resource
        summary_result = await get_repository_summary(temp_dir, context)
...
class TestDataConsistency:
    ...
    async def test_data_consistency_across_operations(self, integration_test_repo):
        ...
        from githound.mcp_server import analyze_repository, RepositoryInput
        repo_input = RepositoryInput(repo_path=temp_dir)
        repo_analysis = await analyze_repository(repo_input, context)
        ...
        metadata_contributors = set(metadata['contributors'])
        history_authors = set(commit['author_name'] for commit in commits)
        stats_authors = set(author_stats.keys())
        
        # All authors in history should be in metadata contributors
        assert history_authors.issubset(metadata_contributors)
        
        # All authors in stats should be in metadata contributors  
        assert stats_authors.issubset(metadata_contributors)
        
        # Verify commit counts in author stats match actual commits
        for author, stats in author_stats.items():
            author_commits = [c for c in commits if c['author_name'] == author]
