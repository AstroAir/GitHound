...
@dataclass
class PermitConfig:
    """Configuration for Permit.io authorization provider."""
...
class PermitAuthorizationProvider(AuthProvider):
    ...
    def __init__(self, base_provider: AuthProvider, **kwargs: Any) -> None:
        ...
        Args:
            ...
            **kwargs: Configuration options for Permit.io
        """
        if not PERMIT_AVAILABLE:
            raise ImportError("permit-fastmcp is required for PermitAuthorizationProvider. Install with: pip install permit-fastmcp")
        
        super().__init__(**kwargs)
        self.base_provider = base_provider
        self.config = PermitConfig(**kwargs)
        self._middleware = None
        self._initialize_permit()
    
    def _load_from_environment(self) -> None:
        """Load Permit.io configuration from environment variables."""
        self.config.permit_pdp_url = os.getenv("PERMIT_MCP_PERMIT_PDP_URL", self.config.permit_pdp_url)
        self.config.permit_api_key = os.getenv("PERMIT_MCP_PERMIT_API_KEY", self.config.permit_api_key)
        self.config.server_name = os.getenv("PERMIT_MCP_SERVER_NAME", self.config.server_name)
        self.config.enable_audit_logging = os.getenv("PERMIT_MCP_ENABLE_AUDIT_LOGGING", "true").lower() == "true"
        self.config.identity_mode = os.getenv("PERMIT_MCP_IDENTITY_MODE", self.config.identity_mode)
        self.config.identity_jwt_secret = os.getenv("PERMIT_MCP_IDENTITY_JWT_SECRET", self.config.identity_jwt_secret)
        ...
        if bypass_methods_env:
            try:
                ...
                logger.warning(f"Invalid JSON in PERMIT_MCP_BYPASSED_METHODS: {bypass_methods_env}")
        ...
        if known_methods_env:
            try:
                ...
                logger.warning(f"Invalid JSON in PERMIT_MCP_KNOWN_METHODS: {known_methods_env}")
    
    def _initialize_permit(self) -> None:
        """Initialize the Permit.io middleware."""
        try:
            if not self.config.permit_api_key:
                raise ValueError("Permit.io API key is required. Set PERMIT_MCP_PERMIT_API_KEY environment variable.")
            
            # Initialize Permit.io middleware
            middleware_config = {
                "permit_pdp_url": self.config.permit_pdp_url,
                "permit_api_key": self.config.permit_api_key,
                "enable_audit_logging": self.config.enable_audit_logging,
            }
            
            if self.config.bypass_methods:
                middleware_config["bypass_methods"] = self.config.bypass_methods
            
            if self.config.known_methods:
            ...
            self._middleware = PermitMcpMiddleware(**middleware_config)
            logger.info(f"Permit.io authorization provider initialized with PDP URL: {self.config.permit_pdp_url}")
    ...
    def update_permit_config(self, **kwargs: Any) -> None:
        """Update Permit.io configuration and reinitialize."""
    ...
    async def check_tool_permission(self, user: User, tool_name: str, tool_args: Dict[str, Any]) -> bool:
        ...
        return await self.check_permission(
            ...
            resource=f"{self.config.server_name}",
            **flattened_args
        )
