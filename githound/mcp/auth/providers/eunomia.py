...
class EunomiaAuthorizationProvider(AuthProvider):
    ...
    def __init__(self, base_provider: AuthProvider, **kwargs: Any) -> None:
        ...
        if not EUNOMIA_AVAILABLE:
            raise ImportError("eunomia-mcp is required for EunomiaAuthorizationProvider. Install with: pip install eunomia-mcp")
        
        super().__init__(**kwargs)
        self.base_provider = base_provider
        self.config = EunomiaConfig(**kwargs)
        self._middleware = None
        self._initialize_eunomia()
    
    def _load_from_environment(self) -> None:
        """Load Eunomia configuration from environment variables."""
        self.config.policy_file = os.getenv("EUNOMIA_POLICY_FILE", self.config.policy_file)
        self.config.server_name = os.getenv("EUNOMIA_SERVER_NAME", self.config.server_name)
        self.config.enable_audit_logging = os.getenv("EUNOMIA_ENABLE_AUDIT_LOGGING", "true").lower() == "true"
    ...
    def _initialize_eunomia(self) -> None:
        ...
        try:
            ...
            if not os.path.exists(self.config.policy_file) and not self.config.custom_policy_data:
            ...
            if self.config.custom_policy_data:
                middleware_config["policy_data"] = self.config.custom_policy_data
            
            self._middleware = create_eunomia_middleware(**middleware_config)
            logger.info(f"Eunomia authorization provider initialized with policy file: {self.config.policy_file}")
    ...
    def _create_default_policy_file(self) -> None:
        ...
        default_policies = {
            ...
            "policies": [
                ...
                },
                {
                    ...
                    "subjects": ["role:user"],
                    "resources": [
                        f"{self.config.server_name}:repository:*",
                        f"{self.config.server_name}:search:*",
                        f"{self.config.server_name}:tools:list"
                    ],
                ...
                },
                {
                    ...
                    "resources": [
                        f"{self.config.server_name}:tools:list",
                        f"{self.config.server_name}:repository:info"
                    ],
                    "actions": ["list", "read"],
                    "effect": "allow"
                },
        ...
        try:
            with open(self.config.policy_file, 'w') as f:
                json.dump(default_policies, f, indent=2)
            logger.info(f"Created default Eunomia policy file: {self.config.policy_file}")
        except Exception as e:
