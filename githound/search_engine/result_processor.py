...
class ResultProcessor:
    ...
    def add_enricher(self, enricher_func: Callable[[SearchResult, SearchContext], SearchResult]) -> None:
    ...
    async def _apply_sorting_and_limiting(
        ...
        if sort_by == "relevance_score":
            ...
            results.sort(
                key=lambda r: r.line_number or 0,
                reverse=(sort_order == "desc")
            )
    ...
    @staticmethod
    def create_context_enricher(context_lines: int = 3) -> Callable[[SearchResult, SearchContext], SearchResult]:
        ...
        def enricher_func(result: SearchResult, context: SearchContext) -> SearchResult:
            ...
            try:
                ...
                file_content = commit.tree[str(result.file_path)].data_stream.read().decode('utf-8', errors='ignore')
