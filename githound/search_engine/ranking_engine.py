...
class RankingEngine:
    ...
    async def _calculate_query_match_score(self, result: SearchResult, query: SearchQuery) -> float:
        ...
        if query.author_pattern and result.commit_info:
            author_text = f"{result.commit_info.author_name} {result.commit_info.author_email}".lower()
            if query.fuzzy_search:
                similarity = fuzz.partial_ratio(
                    query.author_pattern.lower(), 
                    author_text
                ) / 100.0
                score += similarity
            else:
    ...
    async def _calculate_file_importance_score(self, result: SearchResult) -> float:
        ...
        important_extensions = {
            '.py': 0.9, '.js': 0.9, '.java': 0.9, '.cs': 0.9, '.cpp': 0.9, '.c': 0.9,
            '.ts': 0.8, '.go': 0.8, '.rs': 0.8, '.php': 0.8, '.rb': 0.8,
            '.md': 0.7, '.rst': 0.7, '.txt': 0.6,
            '.json': 0.6, '.yaml': 0.6, '.yml': 0.6, '.xml': 0.6,
        ...
        important_files = {
            'readme.md': 0.9, 'readme.txt': 0.9, 'readme.rst': 0.9,
            'changelog.md': 0.8, 'changelog.txt': 0.8,
            'license': 0.7, 'license.txt': 0.7, 'license.md': 0.7,
            'makefile': 0.8, 'dockerfile': 0.8,
            'package.json': 0.8, 'requirements.txt': 0.8, 'setup.py': 0.8,
            'pyproject.toml': 0.8, 'cargo.toml': 0.8,
        }
    ...
    async def _calculate_author_relevance_score(self, result: SearchResult, query: SearchQuery) -> float:
    ...
    async def _calculate_commit_quality_score(self, result: SearchResult) -> float:
        ...
        good_indicators = [
            'fix', 'add', 'update', 'improve', 'refactor', 'implement',
            'feature', 'bug', 'issue', 'enhancement'
        ]
    ...
    async def _calculate_context_relevance_score(self, result: SearchResult, query: SearchQuery) -> float:
    ...
    async def _calculate_frequency_score(self, result: SearchResult, all_results: list[SearchResult]) -> float:
        ...
        same_file_count = sum(
            ...
            if str(r.file_path) == str(result.file_path)
        )
        
        # Count how many results have the same search type
        same_type_count = sum(
            1 for r in all_results 
            if r.search_type == result.search_type
        )
