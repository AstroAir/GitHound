...
class StatisticalSearcher(CacheableSearcher):
    ...
    async def _analyze_commit_patterns(
        self, repo_data: dict[str, Any], context: SearchContext
    ) -> list[SearchResult]:
    ...
    async def _analyze_author_statistics(
        self, repo_data: dict[str, Any], context: SearchContext
    ) -> list[SearchResult]:
        ...
        author_insertions = (
            authors_df.groupby("name")["insertions"].sum().sort_values(ascending=False)
        )
        author_deletions = (
            authors_df.groupby("name")["deletions"].sum().sort_values(ascending=False)
        )
        ...
        author_activity = (
            authors_df.groupby(["name", authors_df["date"].dt.date]).size().reset_index()
        )
    ...
    async def _analyze_file_patterns(
        ...
        # File size analysis
        file_insertions = (
            files_df.groupby("file_path")["insertions"].sum().sort_values(ascending=False)
        )
        file_deletions = (
            files_df.groupby("file_path")["deletions"].sum().sort_values(ascending=False)
        )
