...
class DiffSearcher(CacheableSearcher):
    ...
    async def can_handle(self, query: SearchQuery) -> bool:
        ...
        return (
            (hasattr(query, 'diff_analysis') and bool(query.diff_analysis)) or
            (hasattr(query, 'change_analysis') and bool(query.change_analysis)) or
            (hasattr(query, 'commit_range') and bool(query.commit_range)) or
            any(keyword in (query.text or "").lower() for keyword in ['diff', 'change', 'evolution', 'modified'])
        )
    ...
    async def _analyze_recent_changes(self, context: SearchContext) -> List[SearchResult]:
        ...
        try:
            ...
            # Analyze each commit's changes
            for i, commit in enumerate(commits[:10]):  # Limit to 10 for performance
                try:
                    # Skip merge commits for now
                    ...
                    if change_summary["total_changes"] > 0:
                        ...
                        result = SearchResult(
                            ...
                            relevance_score=1.0 - (i * 0.05),  # Recent commits get higher scores
                            match_context={
            ...
            if results:
                total_files = sum(r.match_context["change_summary"]["files_changed"] for r in results if r.match_context)
                total_insertions = sum(r.match_context["change_summary"]["insertions"] for r in results if r.match_context)
                total_deletions = sum(r.match_context["change_summary"]["deletions"] for r in results if r.match_context)
                ...
                try:
                    ...
                    for diff_item in diff:
                        ...
                        change_info = {
                            ...
                            "insertions": diff_item.insertions if hasattr(diff_item, 'insertions') else 0,
                            "deletions": diff_item.deletions if hasattr(diff_item, 'deletions') else 0,
                        }
