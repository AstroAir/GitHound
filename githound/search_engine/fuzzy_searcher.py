...
class FuzzySearcher(CacheableSearcher):
    ...
    async def can_handle(self, query: SearchQuery) -> bool:
        ...
        return (query.fuzzy_search and 
                (query.author_pattern is not None or 
                 query.message_pattern is not None or
                 query.content_pattern is not None))
    ...
    async def search(self, context: SearchContext) -> AsyncGenerator[SearchResult, None]:
        ...
        self._update_metrics(
            total_commits_searched=len(search_targets),
    ...
    async def _fuzzy_search_authors(self, pattern: str, targets: List[Dict], threshold: float) -> List[SearchResult]:
    ...
    async def _fuzzy_search_messages(self, pattern: str, targets: List[Dict], threshold: float) -> List[SearchResult]:
    ...
    async def _fuzzy_search_content(self, pattern: str, targets: List[Dict], threshold: float) -> List[SearchResult]:
