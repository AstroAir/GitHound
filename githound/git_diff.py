...
class FileDiffInfo(BaseModel):
    ...
    diff_lines: list[DiffLineInfo] = Field(
        default_factory=list, description="Detailed line-by-line diff"
    )
...
def compare_commits(
    repo: Repo, from_commit: str, to_commit: str, file_patterns: list[str] | None = None
) -> CommitDiffResult:
    """
...
def compare_branches(
    repo: Repo, from_branch: str, to_branch: str, file_patterns: list[str] | None = None
) -> CommitDiffResult:
    """
...
def get_file_diff_history(
    repo: Repo, file_path: str, max_commits: int | None = None
) -> list[dict[str, Any]]:
    """
