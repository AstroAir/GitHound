...
class TestAccessibility:
    ...
    async def test_keyboard_navigation(self, page: Page, authenticated_user):
        ...
        assert focused_element in [
            ...
            "BUTTON",
            "A",
        ], "First tab should focus on interactive element"
    ...
    async def test_aria_labels_and_roles(self, page: Page, authenticated_user):
        ...
        assert (
            repo_label or await page.locator('label[for*="repo-path"]').count() > 0
        ), "Repository input should have label or aria-label"
        assert (
            query_label or await page.locator('label[for*="search-query"]').count() > 0
        ...
        assert button_role == "button" or await submit_button.evaluate(
    ...
    async def test_focus_indicators(self, page: Page, authenticated_user):
        ...
        for selector in interactive_elements:
            ...
            has_focus_indicator = (
                ...
                or focus_styles["borderColor"] != "rgba(0, 0, 0, 0)"
            )
    ...
    async def test_form_validation_accessibility(self, page: Page, authenticated_user):
        ...
        if error_count > 0:
            for i in range(error_count):
                ...
                assert role == "alert" or aria_live in [
                    "polite",
                    "assertive",
        ...
        if sr_count > 0:
            for i in range(sr_count):
                ...
                is_sr_only = (
                    ...
                    and styles["left"].startswith("-")
                    or styles["width"] == "1px"
                    and styles["height"] == "1px"
                    or styles["overflow"] == "hidden"
                )
    ...
    async def test_table_accessibility(self, page: Page, authenticated_user, test_repository):
        ...
        if table_count > 0:
            ...
            assert (
                caption > 0 or summary or aria_label
    ...
    async def test_live_regions(self, page: Page, authenticated_user, test_repository):
        ...
        if status_count > 0:
            for i in range(status_count):
                ...
                assert (
                    aria_live or role in ["status", "alert"] or parent_live
