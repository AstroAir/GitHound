...
class TestHelpers:
    ...
    @staticmethod
    async def fill_search_form(page: Page, repo_path: str, query: str, **filters: Any) -> None:
    ...
    async def get_search_result_data(page: Page, index: int = 0) -> Dict[str, str]:
        ...
        return {
            "file_path": await result_card.locator('[data-testid="file-path"]').text_content()
            or "",
            "line_number": await result_card.locator('[data-testid="line-number"]').text_content()
            or "",
            "content": await result_card.locator('[data-testid="code-content"]').text_content()
            or "",
            "commit_hash": await result_card.locator('[data-testid="commit-hash"]').text_content()
            or "",
    ...
    async def check_accessibility_violations(page: Page) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return list(violations) if violations else []
    ...
    async def wait_for_websocket_message(
        ...
        return dict(message_received) if message_received else {}
    ...
    async def wait_for_element_count(
        page: Page, selector: str, expected_count: int, timeout: int = 10000
    ) -> bool:
