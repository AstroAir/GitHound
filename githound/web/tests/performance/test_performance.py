...
class TestPerformance:
    ...
    async def test_page_load_performance(self, page: Page):
        ...
        if performance_metrics["firstContentfulPaint"]:
            assert performance_metrics["firstContentfulPaint"] < 3000, \
    ...
    async def test_large_result_set_performance(self, page: Page, authenticated_user, test_repository):
    ...
    async def test_websocket_performance(self, page: Page, authenticated_user, test_repository):
        ...
        if ws_performance["connectionTime"]:
            assert ws_performance["connectionTime"] < 5000, \
        ...
        if ws_performance["firstMessageTime"]:
            assert ws_performance["firstMessageTime"] < 10000, \
        ...
        try:
            ...
            for i, page in enumerate(pages):
                async def perform_search(p, query_suffix):
                    ...
                    await expect(p.locator('[data-testid="search-results"]')).to_be_visible(timeout=60000)
            ...
            assert search_time < 120.0, f"Concurrent searches took {search_time:.2f}s, exceeds 120 seconds"
    ...
    async def test_scroll_performance(self, page: Page, authenticated_user, test_repository):
        ...
        if total_results > 50:
            assert visible_results <= total_results, "Lazy loading should limit initially rendered results"
