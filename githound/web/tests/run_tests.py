...
class TestRunner:
    ...
    def install_dependencies(self) -> int:
        ...
        exit_code = self.run_command(
            [sys.executable, "-m", "pip", "install", "-e", ".[dev,test]"], cwd=self.project_root
        )
    ...
    def run_auth_tests(self, browser: str = "chromium") -> int:
        ...
        return self.run_command(
            ["pytest", "auth/", f"--browser={browser}", "--html=test-results/auth-test-report.html"]
        )
    ...
    def run_api_tests(self, browser: str = "chromium") -> int:
        ...
        return self.run_command(
            ["pytest", "api/", f"--browser={browser}", "--html=test-results/api-test-report.html"]
        )
    ...
    def run_ui_tests(self, browser: str = "chromium") -> int:
        ...
        return self.run_command(
            ["pytest", "ui/", f"--browser={browser}", "--html=test-results/ui-test-report.html"]
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--headed", action="store_true", help="Run tests in headed mode (visible browser)"
    )
