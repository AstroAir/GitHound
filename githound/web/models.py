...
from ..models import SearchQuery, SearchResult, SearchMetrics, OutputFormat
...
class SearchRequest(BaseModel):
    ...
    branch: Optional[str] = Field(None, description="Branch to search (defaults to current)")
    ...
    timeout_seconds: Optional[int] = Field(300, description="Search timeout in seconds")
...
class SearchResultResponse(BaseModel):
    ...
    matching_line: Optional[str] = Field(None, description="Matching line content")
    ...
    # Match context
    match_context: Optional[Dict[str, Any]] = Field(None, description="Additional match context")
    
    @classmethod
    def from_search_result(cls, result: SearchResult, include_metadata: bool = False) -> "SearchResultResponse":
...
class SearchResponse(BaseModel):
    ...
    def from_results(
        ...
        response_results = [
            SearchResultResponse.from_search_result(r, include_metadata) 
            for r in results
        ]
