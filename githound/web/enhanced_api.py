...
from ..schemas import DataFilter, OutputFormat, SortCriteria
...
class BlameAnalysisRequest(BaseModel):
    ...
    commit: str | None = Field(None, description="Specific commit")
...
class CommitAnalysisResponse(ApiResponse):
    ...
    data: dict[str, Any] | None = Field(None, description="Commit metadata")
...
async def analyze_repository(
    request: RepositoryAnalysisRequest, current_user: dict = Depends(get_current_user)
    ...
    try:
        ...
        return RepositoryAnalysisResponse(
            success=True, message="Repository analysis completed successfully", data=metadata
        )
...
async def analyze_commit(
    request: CommitAnalysisRequest, current_user: dict = Depends(get_current_user)
                            ...
                            file_changes.append(
                                {
                                    ...
                                    "change_type": (
                                        "A"
                                        if diff.new_file
                                        else "M" if diff.a_path == diff.b_path else "R"
                                    ),
                                    "old_path": diff.a_path,
                                    "is_binary": diff.b_blob
                                    and diff.b_blob.size > 1024 * 1024,  # Simple binary check
                                }
                            )
                    commit_data["file_changes"] = file_changes

        return CommitAnalysisResponse(
            success=True, message="Commit analysis completed successfully", data=commit_data
        )
...
async def get_filtered_commits(
    request: CommitFilterRequest, current_user: dict = Depends(get_current_user)
) -> CommitListResponse:
    """
...
async def compare_commits_endpoint(
    request: DiffAnalysisRequest, current_user: dict = Depends(get_current_user)
) -> ApiResponse:
    """
...
async def compare_branches_endpoint(
    request: BranchDiffRequest, current_user: dict = Depends(get_current_user)
) -> ApiResponse:
    """
...
async def get_repository_statistics(
    ...
    try:
        ...
        return ApiResponse(
            success=True, message="Repository statistics retrieved successfully", data=statistics
        )
...
async def analyze_file_blame_endpoint(
    request: BlameAnalysisRequest, current_user: dict = Depends(get_current_user)
) -> ApiResponse:
    """
