...
class ConfigValidator:
    ...
    def validate_config_structure(self, config: Dict[str, Any], context: str, file_path: Path) -> None:
        """Validate configuration structure and values."""
        # Check for known configuration sections
        known_sections = {
            'search', 'cache', 'export', 'web', 'mcp', 'logging',
            'performance', 'aliases', 'export_templates'
        }

        for section in config.keys():
            if section not in known_sections:
                self.warnings.append(f"Unknown config section '{section}' in {file_path} ({context})")
    ...
    def _validate_search_config(self, search_config: Dict[str, Any], context: str, file_path: Path) -> None:
        """Validate search configuration."""
        valid_keys = {
            'max_results', 'fuzzy_threshold', 'include_binary_files',
            'case_sensitive', 'file_patterns', 'exclude_patterns',
            'parallel', 'max_workers', 'timeout'
        }

        for key in search_config.keys():
            if key not in valid_keys:
                self.warnings.append(f"Unknown search config key '{key}' in {file_path} ({context})")

        # Validate specific values
        if 'max_results' in search_config:
            if not isinstance(search_config['max_results'], int) or search_config['max_results'] <= 0:
                self.errors.append(f"max_results must be positive integer in {file_path} ({context})")

        if 'fuzzy_threshold' in search_config:
            threshold = search_config['fuzzy_threshold']
            if not isinstance(threshold, (int, float)) or not 0 <= threshold <= 1:
                self.errors.append(f"fuzzy_threshold must be between 0 and 1 in {file_path} ({context})")

    def _validate_web_config(self, web_config: Dict[str, Any], context: str, file_path: Path) -> None:
        """Validate web server configuration."""
        valid_keys = {
            'host', 'port', 'auto_open_browser', 'cors_origins',
            'auth_enabled', 'rate_limit', 'workers', 'timeout'
        }
    ...
    def _validate_mcp_config(self, mcp_config: Dict[str, Any], context: str, file_path: Path) -> None:
        """Validate MCP server configuration."""
        valid_keys = {
            'host', 'port', 'max_connections', 'enable_auth',
            'rate_limit_enabled', 'auth_required', 'api_key'
        }
        ...
        for key in deprecated_keys:
            if key in mcp_config:
                self.warnings.append(f"Deprecated MCP config key '{key}' in {file_path} ({context}). Use environment variables instead.")

    def _validate_cache_config(self, cache_config: Dict[str, Any], context: str, file_path: Path) -> None:
        """Validate cache configuration."""
        valid_keys = {
            'directory', 'max_size', 'ttl', 'compress',
            'cleanup_interval', 'backend'
        }
        ...
        for yaml_content, context in yaml_examples:
            ...
            if self.validate_yaml_syntax(yaml_content, context, file_path):
                try:
                    ...
                    self.errors.append(f"Error validating config structure in {file_path} ({context}): {e}")
