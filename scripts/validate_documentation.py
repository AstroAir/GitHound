...
class DocumentationValidator:
    ...
    def _validate_anchor_link(
        self, anchor: str, content: str, file_path: Path, line_num: int
    ) -> None:
        ...
        for match in headings:
            ...
            if level > previous_level + 1:
                self.warnings.append(
                    f"{file_path}:{line_num}: Heading level skipped (H{previous_level} -> H{level})"
                )
    ...
    def _validate_code_block_languages(self, content: str, file_path: Path) -> None:
        ...
        for match in matches:
            line_num = content[: match.start()].count("\n") + 1
            self.warnings.append(
                f"{file_path}:{line_num}: Code block missing language specification"
            )
    ...
    def run_validation(self, files: list[Path] | None = None) -> bool:
    ...
    def print_results(self) -> None:
        ...
        if not self.errors and not self.warnings:
            ...
            print(f"\n[OK] No errors found, but {len(self.warnings)} warnings to review.")
        else:
            print(
